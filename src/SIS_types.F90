...
subroutine alloc_fast_ice_avg(FIA, HI, IG, interp_fluxes, gas_fluxes, carbon_fluxes, ice_sheet_enabled)
...
subroutine IOF_chksum(mesg, IOF, G, US, mech_fluxes, thermo_fluxes)
  ...
  if (do_thermo) then
    ...
    if (allocated(IOF%salt_left_behind)) call hchksum(IOF%salt_left_behind, &
          trim(mesg)//" IOF%salt_left_behind", G%HI, unscale=US%S_to_ppt*US%RZ_T_to_kg_m2s)
...
subroutine FIA_chksum(mesg, FIA, G, US, check_ocean)
  ...
  if (allocated(FIA%adot)) call hchksum(FIA%adot, trim(mesg)//" FIA%adot", G%HI, unscale=US%RZ_T_to_kg_m2s)
...
subroutine IST_chksum(mesg, IST, G, US, IG, haloshift)
  ...
  call hchksum(IST%mH_pond_ice, trim(mesg)//" IST%mH_pond_ice", G%HI, haloshift=hs, unscale=US%RZ_to_kg_m2)
